...
function findTicker(text: string): { address: string; name: string } | null {
  const lower = text.toLowerCase();
  for (const [ticker, info] of Object.entries(TOKEN_TICKERS)) {
    if (lower.includes(ticker)) return info;
  }
  return null;
}
...
const EVENT_PATTERNS: { pattern: RegExp; eventName: string }[] = [
...
function detectEvent(text: string): { eventName: string; isDefault: boolean } {
  const lower = text.toLowerCase();
  for (const { pattern, eventName } of EVENT_PATTERNS) {
    if (pattern.test(lower)) return { eventName, isDefault: eventName === "Transfer" && !lower.includes("transfer") };
  }
...
export interface ParseResult {
  ...
  suggestions: string[]; // extra example phrases
}
...
export function parse(input: string): ParseResult {
  ...
  if (!raw) throw new ParseError("Empty input. Try: \"whales over 100k TSLA\"");
  ...
  else if (exactlyMatch) { op = "eq"; amount = parseNumberPhrase(exactlyMatch[2]); }
  ...
  // amount filter
  if (op && amount !== null) {
    conditions.push({ field: "value", op: op as Condition["op"], value: toWeiLike(amount) });
  ...
  if (dest && address) parts.push(`${dest} ${address.slice(0, 10)}…`);
  if (amountDesc) parts.push(amountDesc);
  if (ticker && !amountDesc) parts.push(`on ${tokenName}`);
