...
function getWorker() {
    return new Promise((resolve) => {
        ...
        if (pool.length < POOL_SIZE) {
            const pw = { worker: spawnWorker(), busy: true };
            pool.push(pw);
            resolve(pw);
            return;
        }
...
export function execRegex(pattern, testString) {
...
export async function prewarmRegexPool() {
    const w = [];
    for (let i = 0; i < POOL_SIZE; i++)
        w.push(execRegex("x", "x"));
    await Promise.all(w);
    console.log(`[safe-regex] pool warmed (${pool.length} workers, cap ${POOL_SIZE})`);
}
export async function closeRegexPool() {
