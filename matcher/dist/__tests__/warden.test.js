...
import { evaluate, evaluateWithTrace, validatePredicate, PredicateError, } from "../predicate.js";
...
describe("predicate validation", () => {
    ...
    it("rejects unknown op", async () => {
        expect(() => validatePredicate({ field: "x", op: "bogus", value: "y" })).toThrow(PredicateError);
    });
    ...
    it("rejects too many nested conditions", async () => {
        ...
        for (let i = 0; i < 7; i++) {
            const next = { and: [] };
            current.and.push(next);
            current = next;
        }
        current.and.push({ field: "x", op: "eq", value: "y" });
        expect(() => validatePredicate(deep)).toThrow(PredicateError);
    });
    it("rejects > 32 conditions", async () => {
        const conds = Array.from({ length: 33 }, (_, i) => ({ field: `f${i}`, op: "eq", value: `v${i}` }));
...
describe("plain-english parser", () => {
    it("whales over 100k TSLA", async () => {
        ...
        expect(p.and.some((c) => c.op === "gte" && c.value === toWei(100000))).toBe(true);
    });
    it("transfers under 10 AAPL", async () => {
        const r = parse("transfers under 10 AAPL");
        const p = r.predicate;
        expect(p.and.some((c) => c.op === "lte" && c.value === toWei(10))).toBe(true);
    });
    it("swap over 50000", async () => {
        const r = parse("swap over 50000");
        const p = r.predicate;
        expect(p.and.some((c) => c.field === "eventName" && c.value === "Swap")).toBe(true);
    });
    it("liquidation from 0x1234567890123456789012345678901234567890", async () => {
        ...
        const p = r.predicate;
        expect(p.and.some((c) => c.field === "eventName" && c.value === "Liquidation")).toBe(true);
        expect(p.and.some((c) => c.field === "from")).toBe(true);
    });
    it("to 0xABCDEF123456789012345678901234567890ABCD over 5000", async () => {
        ...
        const p = r.predicate;
        expect(p.and.some((c) => c.field === "to")).toBe(true);
        expect(p.and.some((c) => c.op === "gte" && c.value === toWei(5000))).toBe(true);
    });
    it("deposit over 100k", async () => {
        const r = parse("deposit over 100k");
        const p = r.predicate;
        expect(p.and.some((c) => c.field === "eventName" && c.value === "Deposit")).toBe(true);
    });
    it("mint from 0xabc", async () => {
        const r = parse("mint from 0xaBCDef123456789012345678901234567890ABCD");
        const p = r.predicate;
        expect(p.and.some((c) => c.field === "eventName" && c.value === "Mint")).toBe(true);
    });
    it("burn over 500", async () => {
        const r = parse("burn over 500");
        const p = r.predicate;
        expect(p.and.some((c) => c.field === "eventName" && c.value === "Burn")).toBe(true);
    });
    ...
    it("handles 'k' suffix", async () => {
        ...
        const p = r.predicate;
        expect(p.and.some((c) => c.value === toWei(50000))).toBe(true);
    });
    it("handles 'm' suffix", async () => {
        const r = parse("whales over 2m TSLA");
        const p = r.predicate;
        expect(p.and.some((c) => c.value === toWei(2000000))).toBe(true);
    });
