...
import { ChainListener, ERC20_TRANSFER_ABI } from "./listener.js";
import { evaluateWithTrace } from "./predicate.js";
...
async function main() {
    ...
    listener.on("event", async (flat) => {
        ...
        const deliveries = await Promise.all(matches.map(async ({ sub, trace }) => {
            ...
            console.log(`[matcher] sub ${sub.subId} match -> webhook ${deliveryResult.ok ? "OK" : "FAILED"} ` +
        ...
        for (const { sub, trace, deliveryResult } of deliveries) {
            store.recordDelivery(deliveryResult.deliveryId, sub.subId, deliveryResult.status, deliveryResult.latencyMs, batchTxHash, trace);
        }
    });
    ...
    escrow.on("Subscribed", (subId, subscriber, predicateHash, webhookHash, amount) => {
        console.log(`[matcher] on-chain Subscribed sub=${subId} subscriber=${subscriber} amount=${amount}`);
    });
    escrow.on("Cancelled", (subId) => {
